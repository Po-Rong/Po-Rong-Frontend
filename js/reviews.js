...
async function fetchReviewsData(popupId, sort = 'latest') {
  const reviewsApiUrl = `${window.API_BASE_URL}/popups/${popupId}/reviews?sort=${sort}`;
  const popupDetailUrl = `${window.API_BASE_URL}/popups/${popupId}`;
  const congestionApiUrl = `${window.API_BASE_URL}/popups/${popupId}/congestion`;
...
function renderUpperDashboard(dataList, congestionStats) {
  ...
  const totalScore = dataList.reduce(
    (acc, item) => acc + (item.rating ? parseInt(item.rating) : 0),
    0,
  );
  ...
  const stats = congestionMap[congestionStats.averageCongestionLevel] || {
    label: '-',
    count: 0,
  };
...
function renderReviews(dataList) {
  ...
  dataList.forEach((item) => {
    ...
    // 리뷰 작성일 포맷팅 (createdAt 데이터 처리)
    let formattedCreatedAt = '날짜 없음';
    if (item.createdAt) {
      const datePart = item.createdAt.split(' ')[0];
      const parts = datePart.split('-');
      if (parts.length === 3) {
        formattedCreatedAt = `${parts[0].slice(2)}.${parts[1]}.${parts[2]}`;
      }
    }
    ...
    const congestionData = congestionMap[item.congestionLevel] || {
      label: '정보 없음',
      filledCount: 0,
    };
    ...
    cardArticle.innerHTML = `
            <div class="review-card">
                <div class="review-card-header">
                    ...
                    <div class="review-date" style="display: flex; flex-direction: column; align-items: flex-end; text-align: right; font-size: 13px; color: #888;">
                        <span>작성 : ${formattedCreatedAt}</span>
                        <span>방문 : ${formattedDate}</span>
                    </div>
                </div>
