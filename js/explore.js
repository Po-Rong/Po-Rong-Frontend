...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document
    .getElementById('modal-btn-reset')
    .addEventListener('click', resetTempFilters);
  document
    .getElementById('modal-btn-apply')
    .addEventListener('click', applyFilterChanges);

  // 정렬 드롭다운 변경 시 즉시 재필터링 리렌더링
  document
    .getElementById('popup-sort-select')
...
async function fetchFilteredPopups() {
  ...
  if (currentFilters.regions.length > 0)
    queryString += `&regions=${encodeURIComponent(currentFilters.regions.join(','))}`;
  if (currentFilters.categories.length > 0)
    queryString += `&categories=${encodeURIComponent(currentFilters.categories.join(','))}`;
  ...
  try {
    ...
    popupsData = popupsData.filter((popup) => {
      ...
      if (currentFilters.regions.length > 0) {
        ...
        if (hasSeoulAll) {
          ...
          const isSeoulPopup =
            SEOUL_SUB_REGIONS.includes(popup.regionName) ||
            popup.regionName === '서울 전체';
          // 다른 지역(부산, 대구 등)도 중복 선택했을 수 있으므로 예외 처리 인정
          const isOtherSelectedRegion = currentFilters.regions.includes(
            popup.regionName,
          );
      ...
      if (
        ...
        !currentFilters.categories.includes(popup.categoryName)
      ) {
        return false;
      }
      return true;
    ...
    popupsData.forEach((popup) => {
      ...
      // 백엔드 데이터에 로컬 주소가 하드코딩되어 올 경우를 대비한 동적 치환 예외 처리
...
function resetAllFiltersMaster() {
  ...
  if (chipRegion) {
    chipRegion.textContent = '지역';
    chipRegion.classList.remove('active');
  }
  if (chipCategory) {
    chipCategory.textContent = '카테고리';
    chipCategory.classList.remove('active');
  }
  if (chipStatus) {
    chipStatus.textContent = '운영 상태';
    chipStatus.classList.remove('active');
  }
...
function toggleSubChipSelection(value, element) {
  if (activeModalType === 'status') {
    ...
    document
      .querySelectorAll('#modal-chip-grid .sub-chip')
...
function applyFilterChanges() {
  ...
  if (activeModalType === 'region') {
    ...
    currentFilters.status =
      tempSelectedItems.length > 0 ? tempSelectedItems[0] : '';
