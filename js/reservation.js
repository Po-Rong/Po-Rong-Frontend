document.addEventListener("DOMContentLoaded", function () {
    ...
    async function loadAndPreFillReservation() {
        ...
        if (loginUserStr) {
            ...
            try {
                ...
                const res = await fetch(
                    `${RESERVE_API}/me?user_id=${currentUserId}`,
                );
                if (res.ok) {
                    const list = await res.json();
                    reservation = list.find(
        ...
        if (visitorNameInput)
            visitorNameInput.value = reservation.userName || "";
        if (visitorPhoneInput)
            visitorPhoneInput.value = reservation.userPhone || "";
    ...
    function renderTimeSlots() {
        ...
        if (morningGrid)
            morningGrid.innerHTML = morningTimes
                .map(
                    (t) =>
                        `<button type="button" class="btn-time-slot">${t}</button>`,
                )
                .join("");
        if (afternoonGrid)
            afternoonGrid.innerHTML = afternoonTimes
                .map(
                    (t) =>
                        `<button type="button" class="btn-time-slot">${t}</button>`,
                )
        ...
        document.querySelectorAll(".btn-time-slot").forEach((btn) => {
            btn.addEventListener("click", () => {
                document
                    .querySelectorAll(".btn-time-slot")
    ...
    function setupEventListeners(popupData) {
        ...
        if (dateInput) {
            ...
            const fp = flatpickr("#reserve-date-input", {
                ...
                minDate: minD,
                maxDate: maxD,
                // 1. 기존 예약 값(dateInput.value)이 있다면 유지, 없다면 오늘 날짜(todayStr)를 기본값으로 설정
            ...
            // 2. 초기 로드 시 선택된 날짜 문자열 변수(selectedDateStr)도 업데이트
            selectedDateStr = fp.selectedDates[0]
        ...
        if (reserveBtn) {
            reserveBtn.addEventListener("click", async function () {
                ...
                if (
                    ...
                    alert(
                        "모든 예약 정보를 입력하고 날짜와 시간을 선택해주세요.",
                    );
                    return;
                }

                const loginUserStr = localStorage.getItem("loginUser");
                const loginUser = loginUserStr
                    ? JSON.parse(loginUserStr)
                    : null;
