...
async function fetchTrendPopups() {
  const trendScrollContainer = document.querySelector(
    '.trend-section .card-scroll-container',
  );
  if (!trendScrollContainer) return;

  try {
    const response = await fetch(
      `${API_BASE_URL}/popups?status=ongoing&sort=wishlist${getUserQuery()}`,
    );
    ...
    displayData.forEach((popup, index) => {
      ...
      if (rank === 1) crownHtml = `<div class="crown-badge rank-gold"></div>`;
        ...
        crownHtml = `<div class="crown-badge rank-silver"></div>`;
      else if (rank === 3)
        crownHtml = `<div class="crown-badge rank-bronze"></div>`;
...
async function fetchLeisurePopups() {
  const leisureScrollContainer = document.querySelector(
    '.leisure-section .card-scroll-container',
  );
  if (!leisureScrollContainer) return;

  try {
    const response = await fetch(
      `${API_BASE_URL}/popups?status=ongoing&sort=leisurely${getUserQuery()}`,
    );
    if (!response.ok) throw new Error(`API 통신 에러 발생: ${response.status}`);
...
async function fetchUpcomingPopups() {
  const upcomingScrollContainer = document.querySelector(
    '.upcoming-section .card-scroll-container',
  );
  if (!upcomingScrollContainer) return;

  try {
    const response = await fetch(
      `${API_BASE_URL}/popups?status=upcoming${getUserQuery()}`,
    );
    if (!response.ok) throw new Error(`API 통신 에러 발생: ${response.status}`);
