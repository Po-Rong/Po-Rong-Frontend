document.addEventListener('DOMContentLoaded', () => {
  ...
  if (wishBtn) {
    wishBtn.addEventListener('click', () => {
      ...
      fetch(`${window.API_BASE_URL}/wishlists/popups/${popupId}`, {
        method: 'POST',
        ...
        .then((data) => {
          ...
          return fetch(`${window.API_BASE_URL}/wishlists/popups/${popupId}`);
        })
        .then((res) => res.json())
        .then((allWishList) => {
  ...
  // 팝업 상세 데이터 및 찜 상태 로드
  const fetchPromises = [
    fetch(`${window.API_BASE_URL}/popups/${popupId}`).then((res) => res.json()),
    fetch(`${window.API_BASE_URL}/wishlists/popups/${popupId}`).then((res) =>
      res.json(),
    ),
  ];
  ...
  if (userId) {
    fetchPromises.push(
      fetch(`${window.API_BASE_URL}/wishlists/me?user_id=${userId}`).then(
        (res) => res.json(),
      ),
    );
  }
  ...
  Promise.all(fetchPromises)
    .then((results) => {
      ...
      const popupReservationPeriod = document.getElementById(
        'popup-reservation-period',
      );
      ...
      if (popupPeriod)
        popupPeriod.innerText = `${popup.startDate.split('T')[0]} ~ ${popup.endDate.split('T')[0]}`;
      if (popupLocation) popupLocation.innerText = popup.address;
      if (popupHours)
        popupHours.innerText = popup.operatingHours || '10:00 ~ 22:00';
      if (popupIntro) {
      ...
      if (popupBenefit) {
        const benefitText =
          popup.benefit || popup.benefits || '등록된 혜택이 없습니다.';
      ...
      if (popupSns) {
        ...
        if (snsUrl) {
          ...
          popupSns.parentElement.innerHTML =
            "<span class='meta-value'>등록된 SNS가 없습니다.</span>";
        }
      }
      ...
      if (imageWrapper) {
        ...
        if (images && images.length > 0) {
          ...
          imageWrapper.innerHTML =
            "<p style='color: #999; text-align: center; padding: 20px;'>등록된 상세 이미지가 없습니다.</p>";
        }
      }
      ...
      if (popupRatingScore)
        popupRatingScore.innerText = `${(popup.avgRating || 0).toFixed(1)} / 5.0`;
      if (popupReviewCount) {
      ...
      if (userId && Array.isArray(myWishList)) {
        const isAlreadyWished = myWishList.some(
      ...
      if (reservationBtn) {
        const now = new Date();
        const resStartDate = popup.reservationStartDate
          ? new Date(popup.reservationStartDate.replace(' ', 'T'))
          : null;
        const resEndDate = popup.reservationEndDate
          ? new Date(popup.reservationEndDate.replace(' ', 'T'))
          : null;
