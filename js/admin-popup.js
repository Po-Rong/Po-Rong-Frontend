// 관리자 팝업 등록 JS
const POPUP_API = `${window.API_BASE_URL}/popups`;
...
function loadPopupDetail(id) {
  fetch(`${POPUP_API}/${id}`)
    ...
    .then((data) => {
      ...
      document.getElementById('startDate')._flatpickr.setDate(data.startDate);
      ...
      if (data.info) document.getElementById('popupInfo').value = data.info;
      ...
      if (data.snsUrl) document.getElementById('popupSNS').value = data.snsUrl;
        ...
        Promise.all(
          data.detailImages.map((imageUrl) => {
            ...
            return fetch(fullUrl)
              ...
              .then(
                (blob) =>
                  new File([blob], imageUrl.split('/').pop(), {
                    type: blob.type,
                  }),
              );
          }),
...
document.getElementById('startDate').addEventListener('change', () => {
  const formGroup = document.getElementById('startDate').closest('.form-group');
